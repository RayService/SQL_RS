...
/****** Object:  View [dbo].[TabSTDDenikStandardDataBMView]    Script Date: 04.07.2025 12:53:15 ******/
SET ANSI_NULLS ON
GO
...
CREATE VIEW [dbo].[TabSTDDenikStandardDataBMView]  -- UPDATED 20110318
(OperaceStandard,
Sbornik,
CisloUcet,
CisloUctu01,
...
LEFT OUTER JOIN TabSTDKurzy AS K
ON(((U.TypKurzu=K.TypKurzu)AND((D.DatumPripad_X=(CASE(SELECT KurzPredchozihoDne FROM TabSTDObdobiVykazu
WHERE(TabSTDObdobiVykazu.ID=OV.ID))WHEN 0 THEN K.Datum_X ELSE DATEADD(DAY,1,K.Datum_X)END))
OR((U.TypKurzu<2)AND(K.Datum_X BETWEEN(SELECT ObdobiOd FROM TabSTDObdobiVykazu WHERE(TabSTDObdobiVykazu.ID=OV.ID))
AND(SELECT ObdobiDo FROM TabSTDObdobiVykazu WHERE (TabSTDObdobiVykazu.Id=OV.ID)))))
...
DV.IdStandardOblast,
DV.DatumPripad_X,
CASE SS.MenaBSTD WHEN 0 THEN CONVERT(NUMERIC(19,6),ROUND(DV.Castka*(CASE SP.ZpusobKurzuB WHEN 0 THEN SK.JednotkaMeny/SK.Kurz
ELSE SK.Kurz/SK.JednotkaMeny END),2)*(DV.Strana*-2+1))ELSE CONVERT(NUMERIC(19,6),CastkaMenaFunkZust) END,
RV.DruhyUcet
...
LEFT OUTER JOIN TabSTDKurzy AS SK
ON(((RV.TypKurzu=SK.TypKurzu)AND((DV.DatumPripad_X=(CASE OVyk.KurzPredchozihoDne WHEN 0 THEN SK.Datum_X ELSE DATEADD(DAY,1,SK.Datum_X)END))
OR((RV.TypKurzu<2)AND(SK.Datum_X BETWEEN OVyk.ObdobiOd AND OVyk.ObdobiDo)))
AND(SK.Mena=SP.MenaB)AND(SP.MenaB IS NOT NULL))OR((SK.Id=1)AND(SP.MenaB IS NULL)))
