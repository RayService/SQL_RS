...
/****** Object:  StoredProcedure [dbo].[hpx_UniImport_ZpracujDOBJ_OZ]    Script Date: 26.06.2025 10:13:26 ******/
SET ANSI_NULLS ON
GO
...
/*rpObecneImporty.Import*/CREATE PROC [dbo].[hpx_UniImport_ZpracujDOBJ_OZ]
AS
SET NOCOUNT ON
SET ANSI_NULLS ON
...
WHERE DruhPohybuZbo=53 AND Zaloha=0 AND ISNULL(TypDOBJ,0)=0 AND Chyba IS NULL AND ((@PouzeZaznamyDleLogin=1 AND Autor=SUSER_SNAME())OR(@PouzeZaznamyDleLogin=0)) 
OPEN c
WHILE 1=1
BEGIN
...
EXEC @IdDoklad = [dbo].[hp_DosleObj_NovaHlavicka01]
@Rada=@RadaDokladu,
...
BEGIN
SET @ErrorCode = 0
EXEC [dbo].[hp_DosleObj_NovaTxtPolozka01]
  @IDDoklad=@IdDoklad,
...
END
IF @ChybaPriImportu = 0
UPDATE TabDosleObjTxtPol01 SET Poznamka=ISNULL(TabUniImportOZ.PoznamkaPol, '')
FROM TabUniImportOZ
WHERE TabDosleObjTxtPol01.ID=@IdPolozky AND TabUniImportOZ.ID=@IDImportPol
SET @PopisTxtPol=(SELECT PopisTxtPol FROM TabUniImportOZ WHERE ID=@IDImportPol)
IF @PopisTxtPol IS NOT NULL
  UPDATE TabDosleObjTxtPol01 SET Popis=@PopisTxtPol WHERE ID=@IdPolozky
...
ELSE
  IF @ChybaPriImportu=0
    UPDATE TabDosleObjTxtPol01 SET IDStredNaklad=(SELECT ID FROM TabStrom WHERE Cislo=@StredNakladPol) WHERE ID=@IdPolozky
END
...
ELSE
  IF @ChybaPriImportu=0
    UPDATE TabDosleObjTxtPol01 SET IDNOkruh=(SELECT ID FROM TabNakladovyOkruh WHERE Cislo=@CisloNOkruhPol) WHERE ID=@IdPolozky
END
...
ELSE
  IF @ChybaPriImportu=0
    UPDATE TabDosleObjTxtPol01 SET IDZakazka=(SELECT ID FROM TabZakazka WHERE CisloZakazky=@CisloZakazkyPol) WHERE ID=@IdPolozky
END
...
ELSE
  IF @ChybaPriImportu=0
    UPDATE TabDosleObjTxtPol01 SET IDVozidlo=(SELECT ID FROM TabIVozidlo WHERE EvCislo=@EvCisloPol) WHERE ID=@IdPolozky
END
ELSE
BEGIN
...
ELSE
  IF @ChybaPriImportu=0
    UPDATE TabDosleObjTxtPol01 SET IDZam=(SELECT ID FROM TabCisZam WHERE Cislo=@CisloZamPol) WHERE ID=@IdPolozky
END
END
END
ELSE
...
EXEC [dbo].[hp_DosleObj_NovaPolozka01]
@IdDoklad=@IdDoklad,
...
EXEC dbo.hp_DosleObj_NastavSlevuMnozstviDPHPolozky01
@IDPolozka = @IdPolozky
, @ErrorCode = @ErrorCode OUT
, @ZmenaMnozstvi = 0
, @NewMnozstvi = NULL
...
END
IF @ChybaPriImportu = 0
UPDATE TabDosleObjR01 SET Poznamka=ISNULL(TabUniImportOZ.PoznamkaPol, '')
FROM TabUniImportOZ
WHERE TabDosleObjR01.ID=@IdPolozky AND TabUniImportOZ.ID=@IDImportPol
IF @StredNakladPol IS NOT NULL
BEGIN
IF NOT EXISTS(SELECT * FROM TabStrom WHERE Cislo=@StredNakladPol)
BEGIN
...
ELSE
  IF @ChybaPriImportu=0
    UPDATE TabDosleObjR01 SET IDStredNaklad=(SELECT ID FROM TabStrom WHERE Cislo=@StredNakladPol) WHERE ID=@IdPolozky
END
IF @CisloNOkruhPol IS NOT NULL
BEGIN
IF NOT EXISTS(SELECT * FROM TabNakladovyOkruh WHERE Cislo=@CisloNOkruhPol)
...
ELSE
  IF @ChybaPriImportu=0
    UPDATE TabDosleObjR01 SET IDNOkruh=(SELECT ID FROM TabNakladovyOkruh WHERE Cislo=@CisloNOkruhPol) WHERE ID=@IdPolozky
END
IF @CisloZakazkyPol IS NOT NULL
BEGIN
IF NOT EXISTS(SELECT * FROM TabZakazka WHERE CisloZakazky=@CisloZakazkyPol)
...
ELSE
  IF @ChybaPriImportu=0
    UPDATE TabDosleObjR01 SET IDZakazka=(SELECT ID FROM TabZakazka WHERE CisloZakazky=@CisloZakazkyPol) WHERE ID=@IdPolozky
END
IF @EvCisloPol IS NOT NULL
BEGIN
IF NOT EXISTS(SELECT * FROM TabIVozidlo WHERE EvCislo=@EvCisloPol)
...
ELSE
  IF @ChybaPriImportu=0
    UPDATE TabDosleObjR01 SET IDVozidlo=(SELECT ID FROM TabIVozidlo WHERE EvCislo=@EvCisloPol) WHERE ID=@IdPolozky
END
ELSE
BEGIN
IF @SPZZobrazPol IS NOT NULL
...
ELSE
  IF @ChybaPriImportu=0
    UPDATE TabDosleObjR01 SET IDZam=(SELECT ID FROM TabCisZam WHERE Cislo=@CisloZamPol) WHERE ID=@IdPolozky
END
IF @BarCode IS NOT NULL
BEGIN
  IF EXISTS(SELECT * FROM TabBarCodeZbo WHERE BarCode=@BarCode AND IDKmenZbo=@IDKmen)
    UPDATE TabDosleObjR01 SET BarCode=@BarCode WHERE ID=@IdPolozky
END
ELSE
BEGIN
  UPDATE TabDosleObjR01 SET BarCode=(SELECT TOP 1 BarCode FROM TabBarCodeZbo WHERE IDKmenZbo=@IDKmen AND Prednastaveno=1 ORDER BY Porizeno DESC)
  WHERE ID=@IdPolozky
END
IF @ChybaPriImportu=0
BEGIN
  UPDATE TabDosleObjR01 SET
...
BEGIN
  ...
  END
  ELSE
    UPDATE TabDosleObjR01 SET IDKodPDP=@IdKodPDP WHERE ID=@IdPolozky
END
SET @ChybaExtAtr=''
...
FROM TabUniImportOZ
WHERE TabDosleObjH01.ID=@IDDoklad AND TabUniImportOZ.ID=@IDImport
IF @PopisDodavky IS NOT NULL
  UPDATE TabDosleObjH01 SET PopisDodavky = @PopisDodavky WHERE ID=@IDDoklad
IF @ExterniCislo IS NOT NULL
  UPDATE TabDosleObjH01 SET ExterniCislo = @ExterniCislo WHERE ID=@IDDoklad
IF @DatumDodavky IS NOT NULL
  UPDATE TabDosleObjH01 SET DatumDodavky = @DatumDodavky WHERE ID=@IDDoklad
...
ELSE
  IF @ChybaPriImportu=0
    UPDATE TabDosleObjH01 SET IDStredNaklad=(SELECT ID FROM TabStrom WHERE Cislo=@StredNaklad) WHERE ID=@IDDoklad
END
...
ELSE
  IF @ChybaPriImportu=0
    UPDATE TabDosleObjH01 SET IDNOkruh=(SELECT ID FROM TabNakladovyOkruh WHERE Cislo=@CisloNOkruh) WHERE ID=@IDDoklad
END
...
ELSE
  IF @ChybaPriImportu=0
    UPDATE TabDosleObjH01 SET IDZakazka=(SELECT ID FROM TabZakazka WHERE CisloZakazky=@CisloZakazky) WHERE ID=@IDDoklad
END
...
ELSE
  IF @ChybaPriImportu=0
    UPDATE TabDosleObjH01 SET IDVozidlo=(SELECT ID FROM TabIVozidlo WHERE EvCislo=@EvCislo) WHERE ID=@IDDoklad
END
ELSE
BEGIN
...
ELSE
  IF @ChybaPriImportu=0
    UPDATE TabDosleObjH01 SET IDZam=(SELECT ID FROM TabCisZam WHERE Cislo=@CisloZam) WHERE ID=@IDDoklad
END
IF @DIC IS NOT NULL
BEGIN
...
ELSE
  IF @ChybaPriImportu=0
    UPDATE TabDosleObjH01 SET DIC=@DIC WHERE ID=@IDDoklad
END
IF @MU IS NOT NULL
BEGIN
...
ELSE
  IF @ChybaPriImportu=0
    UPDATE TabDosleObjH01 SET MistoUrceni=@MU WHERE ID=@IDDoklad
END
...
ELSE
  IF @ChybaPriImportu=0
    UPDATE TabDosleObjH01 SET Prijemce=@Prijemce WHERE ID=@IDDoklad
END
...
ELSE
  IF @ChybaPriImportu=0
    UPDATE TabDosleObjH01 SET IDFormaUhrady=(SELECT ID FROM TabFormaUhrady WHERE FormaUhrady=@FormaUhrady) WHERE ID=@IDDoklad
END
...
UPDATE TabDosleObjH01 SET IDFormaDopravy=(SELECT ID FROM TabFormaDopravy WHERE FormaDopravy=@FormaDopravy) WHERE ID=@IDDoklad
END
IF @TerminDodavky IS NOT NULL
UPDATE TabDosleObjH01 SET TerminDodavky=@TerminDodavky WHERE ID=@IDDoklad
SET @ChybaExtAtr=''
EXEC dbo.hpx_UniImport_ZpracujExtAtr
  @ImportIdent=5,
  @IdImpTable=@IdImport,
