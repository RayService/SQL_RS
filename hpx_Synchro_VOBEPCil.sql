...
/****** Object:  StoredProcedure [dbo].[hpx_Synchro_VOBEPCil]    Script Date: 26.06.2025 14:21:16 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[hpx_Synchro_VOBEPCil]
...
-- =============================================
-- Author:		MŽ
-- Description:	Spuštění synchronizace VOB > EP - cílová DB (MC)
-- =============================================

/* deklarace */
DECLARE @CisloOrg INT;
DECLARE @IDPohybRS INT;
...
BEGIN TRY
	...
	'FROM ' + @SQLZdroj + 'dbo.TabPohybyZbozi P'+ @CRLF +
		...
		'LEFT OUTER JOIN ' + @SQLCil + '[dbo].[TabKmenZbozi_EXT] MCtkze ON MCtkze._EXT_RS_IDKmenMC=tkz.ID'+ @CRLF +
		'LEFT OUTER JOIN ' + @SQLCil + '[dbo].[TabKmenZbozi] MCtkz ON MCtkz.ID=MCtkze.ID'+ @CRLF +
	...
	DECLARE CurKontrolaPol CURSOR LOCAL FAST_FORWARD FOR
		SELECT DISTINCT
			IDPohybRS
			FROM #TempGenDok
		WHERE SkupZbo IS NULL AND RegCis IS NULL;
	OPEN CurKontrolaPol;
	FETCH NEXT FROM CurKontrolaPol INTO @IDPohybRS;
		WHILE (@@FETCH_STATUS = 0) AND (@@ERROR = 0)
		BEGIN
			-- zacatek akce v kurzoru CurKontrolaPol
			UPDATE tpze SET _EXT_RS_synchro_problem=1
			FROM RayService..TabPohybyZbozi_EXT tpze
			LEFT OUTER JOIN RayService..TabPohybyZbozi tpz ON tpz.ID=tpze.ID
			WHERE tpze.ID=@IDPohybRS
...
--		AND NOT @ZDokladu = 1
		BEGIN
			...
			VALUES(' Chyba ' + @Error + ', ale vydaná objednávka byla úspěšně synchronizována');
		END;
		END;
	...
	/* ** Založení dokladu - EP ** */
		...
		-- nakopírujeme dokumenty z RS do MC
		...
		SELECT tdRS.Popis,tdRS.JmenoACesta
		FROM RayService.dbo.TabDokumenty tdRS WITH(NOLOCK)
		WHERE ((EXISTS(SELECT*FROM RayService.dbo.TabDokumVazba WHERE IdDok=tdRS.ID AND IdentVazby=9 AND IdTab=@IDDokladZdroj)))
		...
		FETCH NEXT FROM CurZalozPolozky INTO 
			...
			BEGIN
				...
				-- přidáme vazbu na původní pohyb z db MC
				INSERT INTO TabPohybyZbozi_EXT (ID,_EXT_RS_IDPohybMCOriginal)
