...
/****** Object:  StoredProcedure [dbo].[hpx_RS_KopieUmisteniJinySklad]    Script Date: 26.06.2025 16:03:37 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[hpx_RS_KopieUmisteniJinySklad] @IDSklad NVARCHAR(30), @ID INT
AS
...
IF EXISTS (SELECT tu.Id FROM TabUmisteni tu WHERE tu.Kod=@Kod AND tu.IdSklad=@IDSklad)
BEGIN
RAISERROR('Na cílovém skladu již umístění se stejným kódem existuje.',16,1)
RETURN
END;

INSERT INTO TabUmisteni (IdSklad,Kod,Nazev)
...
INSERT INTO TabUmisteni_EXT (ID,_VyjmoutZMapySkladu,_PovolitZaporneMnozNaUmisteni,_IDPolice,_EXT_B2ADIMARS_UseForQaReportInventoryEntryPrint,_EXT_RS_generate_stock_taking_subordinate,_EXT_RS_PhysicalPlace,_EXT_RS_IDLocationPrint,_EXT_RS_VypnoutFIFOFEFO,_EXT_RS_VypnoutVzorec)
