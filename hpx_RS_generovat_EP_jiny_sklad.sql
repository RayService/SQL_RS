...
/****** Object:  StoredProcedure [dbo].[hpx_RS_generovat_EP_jiny_sklad]    Script Date: 26.06.2025 13:07:04 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[hpx_RS_generovat_EP_jiny_sklad] @IDSkladCil NVARCHAR(30)--, @ID INT
AS

/*Označí se EP, pak F9. Pak označit řádky a spustit funkci, která se zeptá na číslo skladu.
Poté se na vybraném skladu založí nový EP ve stejné řadě, jako je hlavička označené položky, převede se položka s množstvím zbývá dodat a veškerými ext.údaji.*/

-- =====================================================================================================================================================================
-- Author:		MŽ
-- Description:	Generování EP na zvoleném skladě z označených řádků více EP. Množství přebírat množství zbývá dodat z původního EP, včetně všech ext.údajů RS.
...
NavaznaObjednavka, FormaUhrady, FormaDopravy, CisloZakazkyDoc, CisloZam, KontaktOsoba, KontaktZam, MnoPrevod, Poradi, IDKmen)
...
tdz.NavaznaObjednavka, tdz.FormaUhrady, tdz.FormaDopravy, tdz.CisloZakazky, tdz.CisloZam, tdz.KontaktOsoba, tdz.KontaktZam, tpz.Mnozstvi-tpz.MnOdebrane, tpz.Poradi, tkz.ID
FROM #TabExtKomID ei
...
/*	-- ** nahození transakce
	...
	*/
	/* ** Založení dokladu - EP ** */
		...
		SET @Insert = 1;
		SET @IDPosta = NULL;
		SET @DruhPohybu = (SELECT DruhPohybuZbo FROM TabDokladyZbozi WHERE ID=@IDDokladZdroj);--dříve bylo 9
		SET @PC = NULL;
		...
		FETCH NEXT FROM CurGenPol INTO 
			...
			BEGIN
				...
				IF (SELECT tpze.ID  FROM TabPohybyZbozi_EXT tpze WHERE tpze.ID = @IDPolozka) IS NULL
				 BEGIN 
					INSERT INTO TabPohybyZbozi_EXT (ID,_PozadovanyIndexZmeny,_LCS_PFA_DuvodOpozdeniExp,_dat_dod)
					VALUES (@IDPolozka,@PozadovanyIndexZmeny,@LCS_PFA_DuvodOpozdeniExp,@dat_dod)
				 END
				ELSE
				UPDATE TabPohybyZbozi_EXT SET _PozadovanyIndexZmeny=@PozadovanyIndexZmeny, _LCS_PFA_DuvodOpozdeniExp=@LCS_PFA_DuvodOpozdeniExp, _dat_dod=@dat_dod WHERE ID = @IDPolozka
...
--	COMMIT;	
	...
	UPDATE TabDokladyZbozi SET Splneno = 1 WHERE ID = @IDDokladZdroj;
