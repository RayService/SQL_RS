...
/****** Object:  StoredProcedure [dbo].[hpx_RS_GenerovaniPozadavkuZDokladu_kalk]    Script Date: 26.06.2025 12:34:25 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[hpx_RS_GenerovaniPozadavkuZDokladu_kalk]
 @Sklad nvarchar(50) = N'200',
 @Sklad2 NVARCHAR(50)='10000115',
 @Oblast_Rez bit = 0,
 @Oblast_EP bit = 1,
 @Oblast_Obj bit = 0,
 @Oblast_DosObj bit = 0,
...
IF (OBJECT_ID('tempdb..#TabPozaZDok_kalk') IS NOT NULL) DROP TABLE #TabPozaZDok_kalk;
--dočasná tabule k porovnání
CREATE TABLE #TabPozaZDok_kalk(
	[ID] [int] IDENTITY(1,1) NOT NULL,
	[Oblast] [tinyint] NOT NULL,
	[IDDoklad] [int] NULL,
	[IDPohyb] [int] NULL,
	[IDPohyb_Pom] [int] NULL,
	[GUIDDosObjH] [binary](16) NULL,
	...
	[IDZakazka] [int] NULL,
	[CisloOrg] [nvarchar](30) NULL,
	[Termin] [datetime] NOT NULL,
	[mnozstvi] [numeric](19, 6) NOT NULL,
	[mnoz_ZadVyp] [numeric](19, 6) NOT NULL,
	[mnoz_Plan] [numeric](19, 6) NOT NULL,
	[mnoz_Prikaz] [numeric](19, 6) NOT NULL,
	[mnoz_VedProd] [numeric](19, 6) NOT NULL,
	[mnoz_sklad] [numeric](19, 6) NOT NULL,
	[Pozadavek] [numeric](19, 6) NOT NULL,
	[DatPorizeni] [datetime] NOT NULL DEFAULT GETDATE(),
	[Autor] [nvarchar](128) NOT NULL DEFAULT SUSER_SNAME())
...
IF @Oblast_Rez=1 OR @Oblast_EP=1 OR @Oblast_Obj=1 OR @Oblast_NabSes=1 OR @Oblast_Kontr=1 
 INSERT INTO #TabPozaZDok_kalk (Oblast, IDPohyb, IDPohyb_Pom, IDZboSklad, IDKmenZbozi, IDZakazModif, IDDoklad, IDZakazka, CisloOrg, mnoz_ZadVyp, mnoz_Plan, mnoz_Prikaz, mnoz_VedProd, mnoz_sklad, Mnozstvi, Pozadavek, Termin) 
  SELECT CASE PZ.druhPohybuZbo WHEN 10 THEN 0 
    ...
    WHERE (@Sklad IS NULL OR 
    ((SS.IDSklad=@Sklad AND DZ.RadaDokladu IN ('320','330','420','430','374')) OR
    (SS.IDSklad=@Sklad2 AND DZ.RadaDokladu IN ('400','410','411','300','310'))))
            AND PZ.Mnozstvi>PZ.MnOdebrane AND 
...
IF NOT EXISTS(SELECT * FROM #TabPozaZDok_kalk) RETURN
/*netřeba zohledňovat výrobní plány, příkazy, ani respektovat sklady a vedlejší produkty
--IF @RespPredbPlan=0 AND @RespPlan=0 AND @RespPrikaz=0 AND @RespSklad=0 AND @RespVedProd=0 RETURN
...
MERGE dbo.TabPozaZDok_kalk AS TARGET
USING #TabPozaZDok_kalk AS SOURCE
ON TARGET.IDPohyb=SOURCE.IDPohyb
--for update
WHEN MATCHED AND (TARGET.Mnozstvi<>SOURCE.Mnozstvi OR TARGET.Termin<>SOURCE.Termin OR TARGET.IDZakazka<>SOURCE.IDZakazka OR TARGET.CisloOrg<>SOURCE.CisloOrg) THEN UPDATE SET  -- to "AND 1=1" je tu proto, že je možnost vkládat další podmínky, nemá vliv na funkci
TARGET.IDZakazka=SOURCE.IDZakazka,
TARGET.CisloOrg=SOURCE.CisloOrg,
TARGET.mnoz_ZadVyp=SOURCE.mnoz_ZadVyp,
TARGET.mnoz_Plan=SOURCE.mnoz_Plan,
TARGET.mnoz_Prikaz=SOURCE.mnoz_Prikaz,
TARGET.mnoz_VedProd=SOURCE.mnoz_VedProd,
TARGET.mnoz_sklad=SOURCE.mnoz_sklad,
TARGET.Mnozstvi=SOURCE.Mnozstvi,
TARGET.Pozadavek=SOURCE.Pozadavek,
TARGET.Termin=SOURCE.Termin,
TARGET.Zmenil=SUSER_SNAME(),
TARGET.DatZmeny=GETDATE()
-- for insert
WHEN NOT MATCHED BY TARGET THEN
INSERT (Oblast, IDPohyb, IDPohyb_Pom, IDZboSklad, IDKmenZbozi, IDZakazModif, IDDoklad, IDZakazka, CisloOrg,
		mnoz_ZadVyp, mnoz_Plan, mnoz_Prikaz, mnoz_VedProd, mnoz_sklad, Mnozstvi, Pozadavek, Termin)
VALUES (SOURCE.Oblast, SOURCE.IDPohyb, SOURCE.IDPohyb_Pom, SOURCE.IDZboSklad, SOURCE.IDKmenZbozi, SOURCE.IDZakazModif, SOURCE.IDDoklad, SOURCE.IDZakazka, SOURCE.CisloOrg,
		SOURCE.mnoz_ZadVyp, SOURCE.mnoz_Plan, SOURCE.mnoz_Prikaz, SOURCE.mnoz_VedProd, SOURCE.mnoz_sklad, SOURCE.Mnozstvi, SOURCE.Pozadavek, SOURCE.Termin)
;
GO
