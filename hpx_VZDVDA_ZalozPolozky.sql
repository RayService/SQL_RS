...
/****** Object:  StoredProcedure [dbo].[hpx_VZDVDA_ZalozPolozky]    Script Date: 26.06.2025 8:40:31 ******/
SET ANSI_NULLS ON
GO
...
/*rpVyuctovaniZalDane.Vyuctovani*/CREATE PROC [dbo].[hpx_VZDVDA_ZalozPolozky] @ID INT
AS
...
-- část I a II
	IF OBJECT_ID(N'tempdb..#VyuctovaniDaneMes') IS NOT NULL
		DROP TABLE #VyuctovaniDaneMes
	...
	INTO #VyuctovaniDaneMes
	FROM TabMzdObd
	WHERE Rok=@Rok
	ORDER BY Mesic ASC

	EXEC hp_MzVyuctovaniDaneZPrijmuFO @Rok, 0
...
-- VyuctovaniDaneZPrijmuFO_06
	...
	WHERE rzd.Rok=@Rok-1 AND rzd.Preplatek>0 AND EXISTS(
							SELECT *
							FROM TabMzSloz m
							WHERE m.CisloMS=97 AND m.ZamestnanecId=rzd.ZamestnanecId AND
								m.IdObdobi IN (SELECT IdObdobi FROM TabMzdObd WHERE Rok=@Rok)
							)
	...
	SELECT @Leden_Castka_06=-SUM(Koruny)
	FROM TabMzSloz
	WHERE CisloMS=97 AND IdObdobi=(SELECT IdObdobi FROM TabMzdObd WHERE Rok=@Rok AND Mesic=1)
	IF @Leden_Castka_06<>0
		SET @Leden_Mesic_06=1
	ELSE
		SET @Leden_Castka_06=NULL

	SELECT @Unor_Castka_06=-SUM(Koruny)
	FROM TabMzSloz
	WHERE CisloMS=97 AND IdObdobi=(SELECT IdObdobi FROM TabMzdObd WHERE Rok=@Rok AND Mesic=2)
	IF @Unor_Castka_06<>0
		SET @Unor_Mesic_06=2
	ELSE
		SET @Unor_Castka_06=NULL

	SELECT @Brezen_Castka_06=-SUM(Koruny)
	FROM TabMzSloz
	WHERE CisloMS=97 AND IdObdobi=(SELECT IdObdobi FROM TabMzdObd WHERE Rok=@Rok AND Mesic=3)
...
/* SouhrnneUdajeCizinci do roku 2016*/
     ...
     BEGIN
	  ...
	  WHERE c.AdrTrvZeme IS NOT NULL AND
		  (c.AdrTrvZeme<>(SELECT ISOKod FROM TabZeme WHERE Vlastni=1)) AND
		  EXISTS(
			  SELECT *
			  FROM TabZamVyp v
			  WHERE v.ZamestnanecId=c.ID AND IdObdobi IN (SELECT IdObdobi FROM TabMzdObd WHERE Rok=@Rok))
	  ...
	  BEGIN
		  ...
		  FROM TabMzdList
		  WHERE ZamestnanecId=@ZamestnanecId AND IdObdobi IN (SELECT IdObdobi FROM TabMzdObd WHERE Rok=@Rok)
...
/* SouhrnneUdajeCizinci od roku 2016*/
     ...
     BEGIN
	  ...
	  SELECT
		  c.ID AS ZamId,
		  ISNULL(c.AdrTrvZeme, N'CZ') AS KodStatu_01,
		  c.Prijmeni AS Prijmeni_02,
		  c.Jmeno AS Jmeno_03,
		  c.AdrTrvMisto AS Obec_04,
		  c.AdrTrvUlice AS Ulice_05,
     ...
     c.RC_C_RC AS RodneCislo_10,
		  CASE WHEN c.CisloPasu = N'' THEN c.CisloOP ELSE c.CisloPasu END AS CisloPasu_11
      ...
      ,CASE WHEN c.CisloPasu = N'' THEN ISNULL(c.AdrTrvZeme, N'CZ') ELSE LEFT(ISNULL(c.PasVydal_Zeme,''), 2) END AS StatDokladu_13
	      ,CASE WHEN c.CisloPasu = N'' THEN N'I' ELSE 'P' END AS TypDokladu_12
	      ,'R' AS TypDIC_10
	  ...
	  BEGIN
		  ...
		  IF @@FETCH_STATUS<>0 BREAK
            ...
            WHERE ZamestnanecId = @ZamestnanecId
                AND IdObdobi IN (SELECT IdObdobi FROM TabMzdObd WHERE Rok = @Rok)
            ...
            SET @UhrnOdmeny_16 = (SELECT SUM(l.UhrnPrijmuZavCin)
                                  ...
                                  WHERE l.ZamestnanecId = @ZamestnanecId
                                    AND l.IdObdobi IN (SELECT IdObdobi FROM TabMzdObd WHERE Rok = @Rok)
...
/* PocetZamestnanci */
	...
	WHERE (TabMzdObd.Rok=@Rok) AND (TabMzdObd.Mesic = 12) 
