...
/****** Object:  StoredProcedure [dbo].[hpx_Balikobot_Validace_cp]    Script Date: 26.06.2025 14:37:56 ******/
SET ANSI_NULLS ON
GO
...
/*rpBalikobot.Export*/CREATE PROC [dbo].[hpx_Balikobot_Validace_cp]
@IdZasilky INT,
...
INSERT #TabExtKom(Poznamka) VALUES(@ErrorMsg)
END
IF (SELECT price FROM Tabx_BalikobotBaliky WHERE OrderNumber=1 AND IdZasilky=@IdZasilky)=0
BEGIN
...
END
END
IF (SELECT service_type FROM Tabx_BalikobotZasilky WHERE ID=@IdZasilky)='RR'
BEGIN
IF EXISTS(SELECT * FROM Tabx_BalikobotBaliky WHERE IdZasilky=@IdZasilky AND ISNULL(weight, 0)>2.00)
BEGIN
SET @ErrorMsg = dbo.hfx_Balikobot_ReturnText('769EA4A1-BEC5-4ABD-8C9A-19FFF91C3105', @Language, DEFAULT, DEFAULT, DEFAULT, DEFAULT)
INSERT #TabExtKom(Poznamka) VALUES(@ErrorMsg)
END
END
END
