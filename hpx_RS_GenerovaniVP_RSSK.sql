...
/****** Object:  StoredProcedure [dbo].[hpx_RS_GenerovaniVP_RSSK]    Script Date: 26.06.2025 13:50:58 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[hpx_RS_GenerovaniVP_RSSK]
@IDPrikaz INT
AS
...
BEGIN
	EXEC @NewIDPrikaz=RayService5.dbo.hp_NewVyrobniPrikaz @IDDilce=@IDDilceNewVP, @kusy_zad=@MnoPrikazRS, @kusy_ciste=@MnoPrikazRS, @Rada='206', /*@IDZakazka=@IDZakazky,*/ @Plan_ukonceni=@TerminVPRS
	--zadání nového VP
...
BEGIN
	...
	SELECT @IDTypOperace = 1--CASE WHEN @TypDilce = 'M' THEN 381 WHEN @TypDilce = 'D' THEN 383 ELSE NULL END
