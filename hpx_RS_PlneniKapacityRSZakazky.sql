...
/****** Object:  StoredProcedure [dbo].[hpx_RS_PlneniKapacityRSZakazky]    Script Date: 26.06.2025 16:06:29 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[hpx_RS_PlneniKapacityRSZakazky]
AS
SET NOCOUNT ON;
--cvičný select
--SELECT resload.RESOURCE_NAME AS ZdrojLPP, resload.BUCKET_END_TIME AS DatumKonec, resload.BUCKET_END_TIME_Y AS DatumKonecR, resload.BUCKET_END_TIME_M AS DatumKonecM, resload.BUCKET_END_TIME_V AS DatumKonecT, 
--resload.LOAD_CAPACITY/60 AS ZakazkyHod,
--SUBSTRING(resload.RESOURCE_NAME,0,CHARINDEX('|',resload.RESOURCE_NAME)) AS Stredisko
...
--SELECT
--tcpo.ID, tcpo.IDTabStrom, tcpo.Pracoviste, CONCAT(tcpo.IDTabStrom,'|',tcpo.Pracoviste)
--FROM TabCPraco tcpo
...
--SELECT IDPracoviste, EP, SUM(Zakazky) AS Zakazky, Rok, Mesic--, Tyden
--FROM #Zakazky
----WHERE NazevZdroje='20021000|KUSOVÁ V.'
--GROUP BY IDPracoviste, NazevZdroje, IDPracoviste, EP, Rok, Mesic--, Tyden
...
DELETE FROM RayService.dbo.Tabx_RS_PrehledKapacitRS WHERE TypKapacity=2

--naplníme naši tabulku
INSERT INTO RayService.dbo.Tabx_RS_PrehledKapacitRS (IDPracoviste, SkupinaPracoviste, TypKapacity, EP, Leden, Unor, Brezen, Duben, Kveten, Cerven, Cervenec, Srpen, Zari, Rijen, Listopad, Prosinec)
...
FROM #Zakazky
WHERE (Rok=DATEPART(YEAR,GETDATE())+1 AND Mesic<@Month) OR (Rok=DATEPART(YEAR,GETDATE()) AND Mesic>=@Month)
...
UPDATE RayService.dbo.Tabx_RS_PrehledKapacitRS SET Leden=Leden*100, Unor=Unor*100, Brezen=Brezen*100, Duben=Duben*100, Kveten=Kveten*100,
