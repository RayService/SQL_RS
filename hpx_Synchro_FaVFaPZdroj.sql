...
/****** Object:  StoredProcedure [dbo].[hpx_Synchro_FaVFaPZdroj]    Script Date: 26.06.2025 9:10:27 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[hpx_Synchro_FaVFaPZdroj]
...
-- =============================================
-- Author:		DJ
-- Description:	Spuštění synchronizace FaV > FaP - zdrojová DB
-- =============================================
...
BEGIN TRY
	...
	-- je to faktura vydaná
	IF (SELECT DruhPohybuZbo FROM TabDokladyZbozi WHERE ID = @IDDokladZdroj) <> 13
		BEGIN
			RAISERROR ('- doklad není typu Faktura vydaná, nelze synchronizovat',16,1);
			RETURN;
		END;
	...
	SET @ExecSQL = N'EXEC ' + @SQLCil + N'[dbo].[hpx_Synchro_FavFaPCil]' + @CRLF +
N'	@DBZdroj' + @CRLF +
...
,@Error OUT;
	...
	IF OBJECT_ID('tempdb..#TabExtKom') IS NOT NULL
		...
		BEGIN
			...
			VALUES(@ParovaciZnak + ' - OK - ' + @HlaskaSynchroCo + '- faktura úspěšně synchronizována');
		END;
