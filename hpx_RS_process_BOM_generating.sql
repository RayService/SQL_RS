...
/****** Object:  StoredProcedure [dbo].[hpx_RS_process_BOM_generating]    Script Date: 26.06.2025 12:11:24 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROC [dbo].[hpx_RS_process_BOM_generating]
@ID INT
AS
-- ============================================================================================
-- Author:		MŽ
-- Create date:            15.6.2021
-- Description:	Generování podřízených procesních dílců
-- ============================================================================================
...
FROM TabCzmeny
WHERE
((TabCZmeny.Platnost = 0 AND TabCZmeny.ID IN  (SELECT zmenaOd FROM tabKVazby WHERE vyssi=@IDHlavicka 
												UNION SELECT zmenaDo FROM tabKVazby WHERE vyssi=@IDHlavicka 
												UNION SELECT zmenaOd FROM tabPostup WHERE dilec=@IDHlavicka
												UNION SELECT zmenaDo FROM tabPostup WHERE dilec=@IDHlavicka
												UNION   SELECT zmenaOd FROM tabNVazby WHERE dilec=@IDHlavicka
												UNION SELECT zmenaDo FROM tabNVazby WHERE dilec=@IDHlavicka 
												UNION   SELECT zmenaOd FROM tabTpvOPN WHERE dilec=@IDHlavicka 
												UNION SELECT zmenaDo FROM tabTpvOPN WHERE dilec=@IDHlavicka
												UNION   SELECT zmenaOd FROM tabVPVazby WHERE dilec=@IDHlavicka 
												UNION SELECT zmenaDo FROM tabVPVazby WHERE dilec=@IDHlavicka
												UNION SELECT zmenaOd FROM tabDavka WHERE IDDilce=@IDHlavicka 
												UNION SELECT zmenaDo FROM tabDavka WHERE IDDilce=@IDHlavicka ))))
...
DECLARE CurGenKVazby CURSOR LOCAL FAST_FORWARD FOR
	...
	WHERE tsze._EXT_RS_generovat_kusovniky=1;
		OPEN CurGenKVazby;
		FETCH NEXT FROM CurGenKVazby INTO 
				@SK;
		...
		SET @SumaCasOperaceRodic=(SELECT SUM(TabPostup.TAC)
								...
								(((TabPostup.dilec=@IDHlavicka AND (@IDZmena>0 AND TabPostup.zmenaOd=@IDZmena OR  @IDZmena=0 AND tcOD.platnostTPV=1 AND tcOD.datum<=GETDATE() AND  (tcDo.platnostTPV=0 OR TabPostup.ZmenaDo IS NULL OR (tcDo.platnostTPV=1 AND tcDo.datum>GETDATE())) ) )))
								)


		EXEC dbo.hp_VytvorPolozkuKmeneZbozi @SZ=@SkupZbo,@RegCis=@RegCis, @Nazev1=@Nazev1,@Nazev2=@NazevSkupZbo,@Dilec=1
		...
		WHERE IDKmenZbozi=@IDDilce
		--lhůta naskladnění
		UPDATE tkz SET tkz.LhutaNaskladneni=@LhutaNaskladneni, tkz.Nazev3=@OznaceniRodic
