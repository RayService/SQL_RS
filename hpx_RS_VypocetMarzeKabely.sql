...
/****** Object:  StoredProcedure [dbo].[hpx_RS_VypocetMarzeKabely]    Script Date: 26.06.2025 13:48:08 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[hpx_RS_VypocetMarzeKabely] @Year INT, @ID INT
AS

IF OBJECT_ID('tempdb..#VypocetMarzeKabely') IS NOT NULL DROP TABLE #VypocetMarzeKabely
CREATE TABLE #VypocetMarzeKabely (
ID INT IDENTITY(1,1),
Year INT NULL,
Month INT NULL,
CisloOrg INT NULL,
...
AND(((tpz.SkupZbo=N'800')OR(tpz.SkupZbo=N'850')OR(tpz.SkupZbo=N'880'))AND(tdz.StredNaklad=N'200'))
AND(tco.CisloOrg=@CisloOrg)
)
INSERT INTO #VypocetMarzeKabely (Year, Month, CisloOrg, VazenyPrumer, CenaCelkem)
...
SELECT *
FROM #VypocetMarzeKabely

DELETE FROM Tabx_RS_MarginsByOrganisations WHERE Year=@Year AND CisloOrg=@CisloOrg AND SalesType=2

INSERT INTO Tabx_RS_MarginsByOrganisations (Year, SalesType, CisloOrg)
VALUES (@Year, 2, @CisloOrg)


--leden
UPDATE tmo SET tmo.Leden=vm.VazenyPrumer,tmo.LedenCena=vm.CenaCelkem
FROM Tabx_RS_MarginsByOrganisations tmo
LEFT OUTER JOIN #VypocetMarzeKabely vm ON vm.Year=tmo.Year AND vm.CisloOrg=tmo.CisloOrg
WHERE tmo.CisloOrg=@CisloOrg AND tmo.Year=@Year AND vm.Year=@Year AND vm.Month=1 AND tmo.SalesType=2
--únor
UPDATE tmo SET tmo.Unor=vm.VazenyPrumer,tmo.UnorCena=vm.CenaCelkem
FROM Tabx_RS_MarginsByOrganisations tmo
LEFT OUTER JOIN #VypocetMarzeKabely vm ON vm.Year=tmo.Year AND vm.CisloOrg=tmo.CisloOrg
WHERE tmo.CisloOrg=@CisloOrg AND tmo.Year=@Year AND vm.Year=@Year AND vm.Month=2 AND tmo.SalesType=2
--březen
UPDATE tmo SET tmo.Brezen=vm.VazenyPrumer,tmo.BrezenCena=vm.CenaCelkem
FROM Tabx_RS_MarginsByOrganisations tmo
LEFT OUTER JOIN #VypocetMarzeKabely vm ON vm.Year=tmo.Year AND vm.CisloOrg=tmo.CisloOrg
WHERE tmo.CisloOrg=@CisloOrg AND tmo.Year=@Year AND vm.Year=@Year AND vm.Month=3 AND tmo.SalesType=2
--duben
UPDATE tmo SET tmo.Duben=vm.VazenyPrumer,tmo.DubenCena=vm.CenaCelkem
FROM Tabx_RS_MarginsByOrganisations tmo
LEFT OUTER JOIN #VypocetMarzeKabely vm ON vm.Year=tmo.Year AND vm.CisloOrg=tmo.CisloOrg
WHERE tmo.CisloOrg=@CisloOrg AND tmo.Year=@Year AND vm.Year=@Year AND vm.Month=4 AND tmo.SalesType=2
--květen
UPDATE tmo SET tmo.Kveten=vm.VazenyPrumer,tmo.KvetenCena=vm.CenaCelkem
FROM Tabx_RS_MarginsByOrganisations tmo
LEFT OUTER JOIN #VypocetMarzeKabely vm ON vm.Year=tmo.Year AND vm.CisloOrg=tmo.CisloOrg
WHERE tmo.CisloOrg=@CisloOrg AND tmo.Year=@Year AND vm.Year=@Year AND vm.Month=5 AND tmo.SalesType=2
--červen
UPDATE tmo SET tmo.Cerven=vm.VazenyPrumer,tmo.CervenCena=vm.CenaCelkem
FROM Tabx_RS_MarginsByOrganisations tmo
LEFT OUTER JOIN #VypocetMarzeKabely vm ON vm.Year=tmo.Year AND vm.CisloOrg=tmo.CisloOrg
WHERE tmo.CisloOrg=@CisloOrg AND tmo.Year=@Year AND vm.Year=@Year AND vm.Month=6 AND tmo.SalesType=2
--červenec
UPDATE tmo SET tmo.Cervenec=vm.VazenyPrumer,tmo.CervenecCena=vm.CenaCelkem
FROM Tabx_RS_MarginsByOrganisations tmo
LEFT OUTER JOIN #VypocetMarzeKabely vm ON vm.Year=tmo.Year AND vm.CisloOrg=tmo.CisloOrg
WHERE tmo.CisloOrg=@CisloOrg AND tmo.Year=@Year AND vm.Year=@Year AND vm.Month=7 AND tmo.SalesType=2
--srpen
UPDATE tmo SET tmo.Srpen=vm.VazenyPrumer,tmo.SrpenCena=vm.CenaCelkem
FROM Tabx_RS_MarginsByOrganisations tmo
LEFT OUTER JOIN #VypocetMarzeKabely vm ON vm.Year=tmo.Year AND vm.CisloOrg=tmo.CisloOrg
WHERE tmo.CisloOrg=@CisloOrg AND tmo.Year=@Year AND vm.Year=@Year AND vm.Month=8 AND tmo.SalesType=2
--září
UPDATE tmo SET tmo.Zari=vm.VazenyPrumer,tmo.ZariCena=vm.CenaCelkem
FROM Tabx_RS_MarginsByOrganisations tmo
LEFT OUTER JOIN #VypocetMarzeKabely vm ON vm.Year=tmo.Year AND vm.CisloOrg=tmo.CisloOrg
WHERE tmo.CisloOrg=@CisloOrg AND tmo.Year=@Year AND vm.Year=@Year AND vm.Month=9 AND tmo.SalesType=2
--říjen
UPDATE tmo SET tmo.Rijen=vm.VazenyPrumer,tmo.RijenCena=vm.CenaCelkem
FROM Tabx_RS_MarginsByOrganisations tmo
LEFT OUTER JOIN #VypocetMarzeKabely vm ON vm.Year=tmo.Year AND vm.CisloOrg=tmo.CisloOrg
WHERE tmo.CisloOrg=@CisloOrg AND tmo.Year=@Year AND vm.Year=@Year AND vm.Month=10 AND tmo.SalesType=2
--listopad
UPDATE tmo SET tmo.Listopad=vm.VazenyPrumer,tmo.ListopadCena=vm.CenaCelkem
FROM Tabx_RS_MarginsByOrganisations tmo
LEFT OUTER JOIN #VypocetMarzeKabely vm ON vm.Year=tmo.Year AND vm.CisloOrg=tmo.CisloOrg
WHERE tmo.CisloOrg=@CisloOrg AND tmo.Year=@Year AND vm.Year=@Year AND vm.Month=11 AND tmo.SalesType=2
--prosinec
UPDATE tmo SET tmo.Prosinec=vm.VazenyPrumer,tmo.ProsinecCena=vm.CenaCelkem
FROM Tabx_RS_MarginsByOrganisations tmo
LEFT OUTER JOIN #VypocetMarzeKabely vm ON vm.Year=tmo.Year AND vm.CisloOrg=tmo.CisloOrg
WHERE tmo.CisloOrg=@CisloOrg AND tmo.Year=@Year AND vm.Year=@Year AND vm.Month=12 AND tmo.SalesType=2

--kvartály
IF OBJECT_ID('tempdb..#VypocetMarzeKabelyQ') IS NOT NULL DROP TABLE #VypocetMarzeKabelyQ
CREATE TABLE #VypocetMarzeKabelyQ (
ID INT IDENTITY(1,1),
Year INT NULL,
Quarter INT NULL,
CisloOrg INT NULL,
...
INSERT INTO #VypocetMarzeKabelyQ (Year, Quarter, CisloOrg, VazenyPrumer, CenaCelkem)
...
SELECT *
FROM #VypocetMarzeKabelyQ

--Q1
UPDATE tmo SET tmo.Q1=vm.VazenyPrumer,tmo.Q1Cena=vm.CenaCelkem
FROM Tabx_RS_MarginsByOrganisations tmo
LEFT OUTER JOIN #VypocetMarzeKabelyQ vm ON vm.Year=tmo.Year AND vm.CisloOrg=tmo.CisloOrg
WHERE tmo.CisloOrg=@CisloOrg AND tmo.Year=@Year AND vm.Year=@Year AND vm.Quarter=1 AND tmo.SalesType=2
--Q2
UPDATE tmo SET tmo.Q2=vm.VazenyPrumer,tmo.Q2Cena=vm.CenaCelkem
FROM Tabx_RS_MarginsByOrganisations tmo
LEFT OUTER JOIN #VypocetMarzeKabelyQ vm ON vm.Year=tmo.Year AND vm.CisloOrg=tmo.CisloOrg
WHERE tmo.CisloOrg=@CisloOrg AND tmo.Year=@Year AND vm.Year=@Year AND vm.Quarter=2 AND tmo.SalesType=2
--Q3
UPDATE tmo SET tmo.Q3=vm.VazenyPrumer,tmo.Q3Cena=vm.CenaCelkem
FROM Tabx_RS_MarginsByOrganisations tmo
LEFT OUTER JOIN #VypocetMarzeKabelyQ vm ON vm.Year=tmo.Year AND vm.CisloOrg=tmo.CisloOrg
WHERE tmo.CisloOrg=@CisloOrg AND tmo.Year=@Year AND vm.Year=@Year AND vm.Quarter=3 AND tmo.SalesType=2
--Q4
UPDATE tmo SET tmo.Q4=vm.VazenyPrumer,tmo.Q4Cena=vm.CenaCelkem
FROM Tabx_RS_MarginsByOrganisations tmo
LEFT OUTER JOIN #VypocetMarzeKabelyQ vm ON vm.Year=tmo.Year AND vm.CisloOrg=tmo.CisloOrg
WHERE tmo.CisloOrg=@CisloOrg AND tmo.Year=@Year AND vm.Year=@Year AND vm.Quarter=4 AND tmo.SalesType=2

--roky
IF OBJECT_ID('tempdb..#VypocetMarzeKabelyY') IS NOT NULL DROP TABLE #VypocetMarzeKabelyY
CREATE TABLE #VypocetMarzeKabelyY (
ID INT IDENTITY(1,1),
Year INT NULL,
CisloOrg INT NULL,
VazenyPrumer NUMERIC (19,6) NULL,
...
INSERT INTO #VypocetMarzeKabelyY (Year, CisloOrg, VazenyPrumer, CenaCelkem)
...
SELECT *
FROM #VypocetMarzeKabelyY

UPDATE tmo SET tmo.YearSum=vm.VazenyPrumer,tmo.YearSumCena=vm.CenaCelkem
FROM Tabx_RS_MarginsByOrganisations tmo
LEFT OUTER JOIN #VypocetMarzeKabelyY vm ON vm.Year=tmo.Year AND vm.CisloOrg=tmo.CisloOrg
WHERE tmo.CisloOrg=@CisloOrg AND tmo.Year=@Year AND vm.Year=@Year AND tmo.SalesType=2
