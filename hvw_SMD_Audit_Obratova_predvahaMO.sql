...
/****** Object:  View [dbo].[hvw_SMD_Audit_Obratova_predvahaMO]    Script Date: 04.07.2025 8:33:29 ******/
SET ANSI_NULLS ON
GO
...
CREATE VIEW [dbo].[hvw_SMD_Audit_Obratova_predvahaMO] AS SELECT dat.Rok,  dat.CisloUcet, u.NazevUctu,
...
(SELECT idObdobi, d.DatumPripad_Y as Rok,  d.CisloUcet,  SUM(CastkaZust) as PocStav, 0 as CastkaMD, 0 as CastkaDal 
 ...
 WHERE d.Stav = 1  -- počáteční stav
    AND Zaknihovano > 0
    AND DatumPripad_X  > =  (SELECT DATEADD(year,-1,DatumOd) FROM SMDTabVyberObdobi) 
    AND DatumPripad_X < = (SELECT DATEADD(year,-1,DatumDo) FROM SMDTabVyberObdobi) 
