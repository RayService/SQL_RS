...
/****** Object:  StoredProcedure [dbo].[hpx_Synchro_PrFaPZdroj]    Script Date: 26.06.2025 9:11:14 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[hpx_Synchro_PrFaPZdroj]
...
-- =============================================
-- Author:		DJ
-- Description:	Spuštění synchronizace Pr > FaP - zdrojová DB
-- =============================================
...
BEGIN TRY
	...
	-- SET @HlaskaSynchroCo = N'Faktura vydaná > Faktura přijatá (' + ISNULL(@DBVerName,N'') + ') '
	...
	-- je to příjemka
	IF (SELECT DruhPohybuZbo FROM TabDokladyZbozi WHERE ID = @IDDokladZdroj) <> 0
		BEGIN
			RAISERROR ('- doklad není typu Příjemka, nelze synchronizovat',16,1);
			RETURN;
		END;
	
	-- kontrola na realizaci faktury
	...
	IF (SELECT CisloOrg FROM Tabx_SynchroConfig WHERE IdentDB = @DBCil) IS NULL
		...
		BEGIN
			RAISERROR ('- organizace dodavatele neodpovídá nulté organizací cílové databáze',16,1);
			RETURN;
		END;
	...
	SET @ExecSQL = N'EXEC ' + @SQLCil + N'[dbo].[hpx_Synchro_PrFaPCil]' + @CRLF +
N'	@DBZdroj' + @CRLF +
...
,@Error OUT;
	...
	IF OBJECT_ID('tempdb..#TabExtKom') IS NOT NULL
		...
		BEGIN
			...
			VALUES(@ParovaciZnak + ' - OK - ' + @HlaskaSynchroCo + '- příjemka úspěšně synchronizována');
		END;
...
BEGIN CATCH
	...
	IF OBJECT_ID('tempdb..#TabExtKom') IS NOT NULL
		...
		BEGIN
			IF NOT EXISTS (SELECT * FROM #TabExtKom WHERE Poznamka LIKE N'Info: Synchronizace dokladů%'
						OR Poznamka LIKE N'Info: Přefakturace%')
				BEGIN
					INSERT INTO #TabExtKom(Poznamka) VALUES(N'Info: Synchronizace dokladů');
					INSERT INTO #TabExtKom(Poznamka) VALUES(N'-------------------------------------------------------------------------------');
				END;
