...
/****** Object:  StoredProcedure [dbo].[hpx_Balikobot_Validace_toptrans]    Script Date: 26.06.2025 15:01:31 ******/
SET ANSI_NULLS ON
GO
...
/*rpBalikobot.Export*/CREATE PROC [dbo].[hpx_Balikobot_Validace_toptrans]
@IdZasilky INT,
...
INSERT #TabExtKom(Poznamka) VALUES(@ErrorMsg)
END
IF (SELECT ISNULL(rec_phone, '') FROM Tabx_BalikobotZasilky WHERE ID=@IdZasilky)=''
BEGIN
SET @ErrorMsgParam1='"' + dbo.hfx_Balikobot_ReturnText('F5288193-8DB2-4F2F-9020-DBC8358BD305', @Language, DEFAULT, DEFAULT, DEFAULT, DEFAULT) + '"'
SET @ErrorMsg = dbo.hfx_Balikobot_ReturnText('F26E2F23-F45F-43EC-82C4-D27171D9E0F3', @Language, @ErrorMsgParam1, DEFAULT, DEFAULT, DEFAULT)
INSERT #TabExtKom(Poznamka) VALUES(@ErrorMsg)
END
...
END
END
IF (SELECT COUNT(DISTINCT mu_type) FROM Tabx_BalikobotBaliky WHERE mu_type IS NOT NULL AND IdZasilky=@IdZasilky)>3
BEGIN
SET @ErrorMsg = dbo.hfx_Balikobot_ReturnText('84D1B916-34A5-4CC5-888E-3C1A24F0693B', @Language, DEFAULT, DEFAULT, DEFAULT, DEFAULT)
INSERT #TabExtKom(Poznamka) VALUES(@ErrorMsg)
END
IF EXISTS(SELECT * FROM Tabx_BalikobotBaliky WHERE IdZasilky=@IdZasilky AND ISNULL(mu_type, '')='')
BEGIN
SET @ErrorMsg = dbo.hfx_Balikobot_ReturnText('1DA852C6-BF2C-49E8-82C0-9879BADC1E7F', @Language, DEFAULT, DEFAULT, DEFAULT, DEFAULT)
INSERT #TabExtKom(Poznamka) VALUES(@ErrorMsg)
END
IF EXISTS(SELECT * FROM Tabx_BalikobotBaliky WHERE IdZasilky=@IdZasilky AND ISNULL(weight, 0)=0)
BEGIN
SET @ErrorMsg = dbo.hfx_Balikobot_ReturnText('D933803F-F53E-4FEA-814D-4F6A1FECFBAF', @Language, DEFAULT, DEFAULT, DEFAULT, DEFAULT)
INSERT #TabExtKom(Poznamka) VALUES(@ErrorMsg)
END
GO
