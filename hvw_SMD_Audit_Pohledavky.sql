...
/****** Object:  View [dbo].[hvw_SMD_Audit_Pohledavky]    Script Date: 04.07.2025 8:33:57 ******/
SET ANSI_NULLS ON
GO
...
CREATE VIEW [dbo].[hvw_SMD_Audit_Pohledavky] AS SELECT * FROM
...
(SELECT DatumPripad_X as DatumPripad, CisloUcet, CastkaMD, CastkaDAL, CastkaZust, UcetDPH, d.Mena, Kurz, CastkaMenaZust, DatumPripad_M, DatumPripad_Y, DatumDUZP_X as DatumDuzp,
 ...
 CASE WHEN D.Strana = 0 THEN CAST(Sbornik as NVARCHAR)+'/'+CAST(CisloDokladu as NVARCHAR)  ELSE NULL END as CisloDokladu,
 ...
 CASE WHEN d.DruhData IN (1,3) THEN CastkaZust  ELSE 0 END as CastkaSplatno,
 CASE WHEN d.DruhData = 2 THEN CastkaZust * -1  ELSE 0 END as CastkaUhrazeno,
 CASE WHEN d.DruhData IN (1,3) THEN CastkaMenaZust  ELSE 0 END as CastkaSplatnoMena,
 CASE WHEN d.DruhData = 2 THEN CastkaMenaZust * -1  ELSE 0 END as CastkaUhrazenoMena
 FROM TabDenik d
 LEFT JOIN TabCisOrg o ON o.CisloOrg = d.CisloOrg 
 WHERE Zaknihovano > 0) as denik

WHERE LEFT(CisloUcet,3) = '311' 
