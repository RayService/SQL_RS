...
/****** Object:  View [dbo].[TabPohybySNHlaView]    Script Date: 04.07.2025 11:37:00 ******/
SET ANSI_NULLS ON
GO
...
CREATE VIEW [dbo].[TabPohybySNHlaView] AS
SELECT T.ID,
...
FROM (SELECT D.ID, MIN(P.DruhPohybuZbo) AS DP,
...
JOIN TabPohybyZbozi  AS P ON P.IDDoklad = D.ID
LEFT OUTER JOIN TabPohybySN  AS O ON O.IDPohyb = P.ID
LEFT OUTER JOIN TabDokladySN AS S ON S.ID = O.IDDoklad AND S.Realizovano = 1
GROUP BY D.ID) AS T
GO
