...
/****** Object:  StoredProcedure [dbo].[hpx_RS_calculation_CalculationMask]    Script Date: 26.06.2025 11:43:54 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[hpx_RS_calculation_CalculationMask] @KalkKurzVal NUMERIC(19,6), @KalkSazba NUMERIC(19,6) ,@KalkPriraz INT , @KoefObch NUMERIC(19,6), @ID INT
AS
SET NOCOUNT ON
...
--MŽ, 7.7.2022 opět upraveno na dohledání vazby mezi položkou pro kalkulaci a maskou dle ID položky pro kalkulaci
--deklarace pro řádek položky
DECLARE @Dilec INT,@IDZakazka INT,@MnoPolPoz NUMERIC(19,6),@IDSkladPoz INT, @Prumer NUMERIC(19,6), @NasobitelMzdy NUMERIC(19,6), @NasobitelRezie NUMERIC(19,6),
...
SELECT @Dilec=tpzk.IDKmenZbozi, @IDZakazka=IDZakazka, @MnoPolPoz=tpzk.mnozstvi, @IDSkladPoz=tpzk.IDZboSklad, @Prumer=tss.Prumer
...
SELECT @NakladovaCena=((ISNULL(tzk.mat,0)+ISNULL(tzk.OPN,0)+ISNULL(tzk.koop,0))+(ISNULL((tzk.TAC_KC+tzk.TBC_KC),0)*@NasobitelMzdy)+(ISNULL((tzk.rezieS),0)*@NasobitelRezie))
FROM Tabx_RS_ZKalkulace tzk
LEFT OUTER JOIN TabZakazka tz ON tz.CisloZakazky=tzk.CisloZakazky
WHERE tzk.Dilec=@Dilec AND tz.ID=@IDZakazka
...
WHERE IDPozadavku=@ID--dříve bylo (Dilec=@Dilec) AND (IDZakazka=@IDZakazka) AND (MnozstviPoz=@MnoPolPoz)
END
ELSE
BEGIN
INSERT INTO Tabx_RS_KalkulaceObchodni (IDPozadavku,Dilec,IDZakazka,MnozstviPoz,NakladovaCena,MinCena,KalkCena,Material,TAC,TBC,OptCena,VyslNabCena,TerminDodani,EvidCena,HistProdCenaObd
,KalkKurzVal,KalkSazba,KalkPriraz,KoefObch,NasMinCena,NasOptCena,DnyHistorie,IDSklad,NasobitelMzdy,NasobitelRezie,MzdaCista,RezieCista) 
VALUES (@ID,@Dilec,@IDZakazka,@MnoPolPoz,@NakladovaCena,@MinCena,@KalkCena,@Material,@TAC,@TBC,@OptCena,ROUND(@VyslNabCena,2),@TerminDodani,@EvidCena,@HistProdCenaObd
,@KalkKurzVal,@KalkSazba,@KalkPriraz,@KoefObch,@NasMinCena,@NasOptCena,@DnyHistorie,@IDSklad,@NasobitelMzdy,@NasobitelRezie,@MzdaCista,@RezieCista)
END
